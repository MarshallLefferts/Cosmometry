/*AssetCacher names: javascripts/jquery.js | javascripts/jquery.lightbox-0.5.js | javascripts/galleria/galleria.js | javascripts/galleria/themes/classic/galleria.classic.js | parameters | application | javascripts/jquery.cookie.js | javascripts/jquery.simplemodal-1.4.2.js */
/*!
...
var Picture = Base.extend({
    ...
    // Line changed by Miles Romney
    load: function(src, orig_src, callback) {
        callback = this.proxy( callback );
        this.elem.innerHTML = '';
        ...
        this.wait(function() {
            ...
            // Line changed by Miles Romney
            callback( {target: this.image, scope: this, orig: orig_src} );
        }, function() {
...
var G = window.Galleria = Base.extend({
    ...
    run : function() {
        ...
        for( var i=0; this.data[i]; i++ ) {
            ...
            if (o.thumbnails === true) {
                ...
                // Line changed by Miles Romney
                thumb.load(src, this.data[i].image, this.proxy(function(e) {
                    var orig = this.width(e.target);
                    // Block added by Miles Romney
                    if(e.orig != undefined && e.orig.search('.mp4') > 0) {
                      $(e.target).parent().prepend('<div class="video_thumb"> </div>');
                    }
    ...
    // Method added by Miles Romney
    imageToLoad : function(src) {
      var isVideo = (src.match(/\.(flv|m4v|mp4)/i) ? true : false);

      if(window.GALLERIA_IMAGE_JUST_LOADED == undefined) {
        window.GALLERIA_IMAGE_JUST_LOADED = {};
      }
      window.GALLERIA_IMAGE_JUST_LOADED[this.options.instance] = src;
      
      return (isVideo ? this.options.video_slate : src);
    },
    ...
    showImage : function() {
        ...
        // Line modified by Miles Romney
        next.load( this.imageToLoad(src), undefined, this.proxy(function(e) {
        ...
        if (
            ...
            var getData = this.proxy(function( elem ) {
                ...
                if (anchor && anchor.nodeName == 'A') {
                    // Line changed by Miles Romney to add video support
                    if (anchor.href.match(/\.(png|gif|jpg|flv|m4v|mp4)/i)) {
                        i = anchor.href;
                    } else {
                        j = anchor.href;
                    }
...
jQuery(window).load(function(){
 ...
 var homeHeight = $('#home-content').height();
  if ( $('#home-sidebar-content').height() > $('#home-content').height() ) {
    $('#home-sidebar-content').css('height','325px');
  }
  else{
    $('#home-sidebar-content').css({'height': (homeHeight - 120) + 'px'})
  }

var landingHeight = $('#landing-content').height() - 90;
var landingSidebarHeight = $('.landing-sidebar').height();

  if ( $('#landing-sidebar-content').height() > landingHeight ) {
    $('#landing-sidebar-content').css({'height': (landingSidebarHeight) + 'px'});
  }
  else{
     $('#landing-sidebar-content').css({'height': (landingHeight - 30) + 'px'});
  }
...
/* Added such that resize is possible via setTimeout for Disqus Comments */

function resizeSidebar() {
    var landingHeight = $('#landing-content').height() - 80;
    var landingSidebarHeight = $('.landing-sidebar').height();
    ...
    if ( $('#landing-sidebar-content').height() >= landingHeight ) {
      $('#landing-sidebar-content').css({'height': (landingSidebarHeight) + 'px'});
       ...
       $('#landing-sidebar-content').css({'height': (landingHeight) + 'px'});
    }
  }

function initSidebarMenu() {
  $('#landing-sidebar-content .landing-sidebar-p').append('<div class="sidebar-menu"><div class="node" name="root"></div></div>');

  $(window.sidebarNodes).each(function() {
    
    if (! /\//.test(this) ) {
      $('.sidebar-menu .node[name="root"]').append('<div class="node shadowed" name="' + this + '" path="' + this + '" lineage="' + this + '"><div class="parentNodeTitle"><span class="resource" resource="' + createPermalink(this) + '">' + this + '</span></div></div>');
    } else {
      
      var nodes = this.split(" / ");
      var thisNode = nodes[nodes.length - 1];
      var parentNode = nodes[nodes.length - 2];
      
      $('.sidebar-menu .node[name="' + parentNode + '"]').append('<div class="node" name="' + thisNode + '" path="' + thisNode + '" lineage="' + thisNode + '"><div class="withChildrenWrapper"><span class="resource inner" resource="' + createPermalink(thisNode) + '">' + thisNode + '</span></div></div>');
...
//  $(window.sidebar_menu_data).each(function() {
//  	var parent = this['parent'] || 'root';
//	
//  	$('.sidebar-menu .node[name="' + parent + '"]').append('<div class="node" name="' + this['name'] + '" path="' + this['id'] + '" lineage="' + this['lineage'] + '"><span class="resource" resource="' + this['resource'] + '">' + this['name'] + '</span></div>');
//  });
//
  $('.sidebar-menu div[name=root]').show();
  $('.sidebar-menu div[name=root] > .node').show();

  $('div.node[name="root"] div.node:first-child .parentNodeTitle:first-child').addClass('titleBox');
  $('div.node[name="root"] div.node:nth-child(2) .parentNodeTitle').addClass('titleBox');
  $('div.node[name="root"] div.node:nth-child(3) .parentNodeTitle').addClass('titleBox');

  $('div.node[name="root"] > div.node > .parentNodeTitle').each(function() {
    $(this).addClass('titleBox');
  });

  $('.sidebar-menu div[name=root] .node').each(function(){
  	if($(this).find('.node').size() > 0) { 
          if($(this).find('.titleBox').size() > 0) { 
            $(this).find('.titleBox').append('<a class="nav" href="javascript:void(0);"><span class="icon"><img src="images/sidemenu_triangle_upside_down.png"></span><span class="icon" style="display: none;"><img src="images/sidemenu_triangle.png"></span> </a>');
          } else { $(this).addClass('hasChildren');
  		$(this).prepend('<a class="nav" href="javascript:void(0);"><span class="icon">+</span><span class="icon" style="display: none;">-&nbsp;</span> </a>');
         }
  	} else {
  ...
  $('.sidebar-menu div[name=root] .node a').each(function(){
  	
        $(this).click(function(){
             if ($(this).parent().hasClass('titleBox')) {
               $(this).parent().parent().find('> .node').toggle();
  	       $(this).parent().find('> a .icon').toggle(); 
             } else {
  ...
  $('.sidebar-menu div[name=root] .node .resource').each(function(){
  	if($(this).attr('resource') != 'null') {
  		$(this).click(function(){
  			document.location.href = '/' + $(this).attr('resource');
  		});
  	}
  });

  /* Toggle default selection */
  $default = $('.sidebar-menu span[resource=' + window.articlePermalink + ']');
  $parents = $default.parents('.node:not([name=root])');
  $parents.show();
  $parents.find('> a .icon').toggle();
  $default.parents('.node:not([name=root])').last().find('.titleBox > a .icon').toggle();
  $parents.find('> .node').show();
  if ($parents.length != 1) {
    if ($default.parent().hasClass('withChildrenWrapper')) {
      $default.parent().addClass('sidebarSelected');
    } else {  
    $parents.first().addClass('sidebarSelected');
    }
  } else {
  ...
  $('.breadcrumbs').append('&nbsp;&nbsp;<span class="breadcrumbCarrot">></span>&nbsp;&nbsp;<a class="breadcrumb-link" href="/landing?tags=' + createPermalink(window.breadcrumbSectionTitle) + '">' + window.breadcrumbSectionTitle + '</a>');
  $($default.parents('.node:not([name=root])').get().reverse()).each(function(){
    $('.breadcrumbs').append('&nbsp;&nbsp;<span class="breadcrumbCarrot">></span>&nbsp;&nbsp;<a class="breadcrumb-link" href="' + $(this).find('.resource').attr('resource') + '">' + $(this).attr('name') + '</a>');
  });
  
  if($default.parents('.node').size() > 1) {
    $('#landing-sidebar').prepend('<div class="section-title">' + $('div.breadcrumbs .breadcrumb-link:nth-child(3)').html() + '</div>');
   if ($('.section-title').height() <= 28) {
      $('.section-title').css('top', '150px');
   }
  }
}

$(function(){
  /* splash screen disabled by Marshall 131106 */
   //if ($.cookie("first_cosmometry_visit") == null) {
   //   $('#splash').bind('click', function() { $.modal.close(); });
   //   $('#splash').modal({overlayClose:true});
   //   $.cookie("first_cosmometry_visit", "nope", { expires: 365 });
   //}
  ...
  if($('#article-handle').size() > 0) {
    ...
    $('a.nav').bind('click', function() {
      $('#landing-sidebar-content').css('height', ($('.landing-sidebar-p').height() + 75) + 'px');
      $('#landing-sidebar').css('height', ($('.landing-sidebar-p').height() + 165) + 'px');
      resizeSidebar(); 
    });
  
 
  $('.node > .node > .node span.resource').each(function() {
    $(this).css('padding-left', '8px');
    $(this).css('font-weight', 'bold');  
  });

  $('.node > .node > .node > .node span.resource').each(function() {
  ...
  $('.node > .node > .node span.resource').each(function() {
    var label = $(this).html();
    if (label.length > 30) { 
      var indent = '<br><span class="spacer"></span>';
      for(var i = 30; i > 0; i--) {
        if (label.charAt(i) == ' ') { 
          label = label.substr(0, i) + indent + label.substr(i);
          $(this).html(label);
          break;
        }
  ...
  $('.node > .node > .node > a.nav').each(function() { 
     $(this).css('position', 'absolute'); 
     $(this).css('left', '23px');
  });

  $('.parentNodeTitle').each(function() { 
    $(this).parent().find('> .node:first').css('margin-top', '8px');
    $(this).parent().find('> .node:last').css('padding-bottom', '6px');
  });


  $('.parentNodeTitle.titleBox span.icon').each(function() {
    $(this).css('float', 'right');
    $(this).css('margin-right', '6px');
  });

  $('.parentNodeTitle').hover(function() { 
     $(this).css('color', '#001CBA') 
   }, function() {
     $(this).css('color', '#60718D')
   });

$('.node > .node > .node > .node').hover(function() { 
    $(this).addClass('sidebarHover'); 
  }, function() { 
    $(this).removeClass('sidebarHover'); 
  });

$('.node > .node > .node:not(.hasChildren)').hover(function() {
    $(this).addClass('sidebarHover'); 
  }, function() { 
    $(this).removeClass('sidebarHover'); 
  });
 
$('.node > .node > .node.hasChildren > .withChildrenWrapper').hover(function() {
  $(this).addClass('sidebarHover');
  }, function() {
    $(this).removeClass('sidebarHover');
  });
